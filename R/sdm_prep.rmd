...
Point data are helpful for describing specific occurrences of a species, but they often come with biases (e.g., there may be many more records in easily accessible places). Thus, we often also need to rely on maps drawn by experts. The IUCN makes such maps available for free (https://www.iucnredlist.org/resources/spatial-data-download), and there are many other such sources online. Access to data is provided for approved reasons; our data is available for you to download from OLAT. I have saved it in the "data" folder inside my working directory.
...
We will use the (bioclim)[http://www.worldclim.org/bioclim] climate variables for our analysis; be sure to read over the link for information on what these variables represent and why they are useful in SDM applications. We get 19 (!) variables in bioclim.
...
We have already set the appropriate resolution using the `res` argument. However, the `raster` package provides a lot of functions that are useful for manipulating these data in case we need to change resolutions, extents, etc. It is useful to brows the help for these to know they are available:
