...
mamm = readRDS("../data/mammal_ranges.rds")
mamm_r = fasterize(mamm, clim_pres[[1]], fun = "sum", background = NA)
mamm_r_status = fasterize(mamm, clim_pres[[1]], fun = "sum", background = NA, by="category")
plot(mamm_r, col=scico::scico(100))

## sum the richness of any vulnerable/threatened/endangered species
names(mamm_r_status) ## we want index 2,4,5,6
mamm_en = raster::stackApply(mamm_r_status, c(1,2,1,2,2,2), sum)[[2]]
...
-- elevation (https://www.eea.europa.eu/data-and-maps/data/copernicus-land-monitoring-service-eu-dem)
...
First, we start with `bio1`, the mean annual temperature. This is a nice measure of the overall warmness of the climate. We can eliminate any variable with a correlation >= 0.5:

```{r bio1}
thresh = 0.5
chosen = 'bio1'
## what is left
(remaining = names(which(cor_mat['bio1',] < thresh)))
```

We would like a preciptation measure as well; I propose summer precipitation as being most relevant for the shrew.

```{r bio18}
chosen = c(chosen, 'bio18')
(remaining = names(which(cor_mat['bio18',remaining] < thresh)))
```

Now that we have estimates of location (i.e., means or totals), we could also examine climatic variability. For temperature, we choose `bio4`, seasonality, for precipitation we go with `bio15`.

```{r bio415}
chosen = c(chosen, 'bio4')
(remaining = names(which(cor_mat['bio4',remaining] < thresh)))
chosen = c(chosen, 'bio15')
(remaining = names(which(cor_mat['bio15',remaining] < thresh)))
```


We still have `bio2` (diurnal range), and `bio19` (winter precip). They are not strongly correlated, so we keep both.

```{r bio219}
cor_mat[remaining,remaining]
(chosen = c(chosen, 'bio2', 'bio19'))
...
```{r full_model}
full_mod = glm(presence ~ bio1 + bio2 + bio4 + bio15 + bio18 + bio19 + 
			   	I(bio1^2) + I(bio2^2) + I(bio4^2) + I(bio15^2) + I(bio18^2) + I(bio19^2),
...
```{r addbio19}
## adding bio4 back in
AIC(reduced_mod) - AIC(update(reduced_mod, ~.+bio19))

## dropping the quadratic term
AIC(reduced_mod) - AIC(update(reduced_mod, ~.-I(bio19^2)))

reduced_mod = update(reduced_mod, ~.+bio19)
```

We see the same with `bio19.` So now, our model looks like this. Note that this model is quite large, and contains curvilinear terms; thus it is not so easy to understand just from the parameters how the range of our shrew relates to each predictor.
...
vars = c("bio1", "bio2", "bio4", "bio15", "bio18", "bio19")
nms = c(bio1 = "Annual Mean Temperature", bio2 = "Mean Diurnal Range", bio4 = "Temperature Seasonality", 
		bio15 = "Precipitation Seasonality", bio18 = "Precipitation of Warmest Quarter", bio19 = "Precipitation of Coldest Quarter")

## create a dummy data frame for predicting
pr_data = data.frame(bio1 = rep(0, 100), bio2=0, bio4=0, bio15=0, bio18=0, bio19=0)
par(mfrow=c(2,3))
for(v in vars) {
