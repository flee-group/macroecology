...
The final step, joining the data, is trivial. The rasters are already aligned and in the same format, so we just use the `c` function to collapse them into a single dataset, and then `as.data.frame` to put them into a data.frame for analysis.
...
m1_p = pts + geom_line(data = prdat, aes(x=bio12, y=richness), linewidth = 1.4, colour = cols[1])
m1_p
```

This model certainly seems inadequate. There is very high scatter around the line, and the relationship seems highly nonlinear. Interestingly, we see an increase in richness with precipitation.
...
Less than one, so we can at least assume the dispersion of our model is quite unappropriate. What could we do?
